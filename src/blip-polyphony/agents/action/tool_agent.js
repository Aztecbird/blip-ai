...
class ToolAgent {
  constructor(options = {}) {
    this.appHandlers = options.appHandlers || {};
  }
  ...
  async process(capsule) {
    ...
    for (const step of capsule.plan) {
      ...
      try {
        ...
        // 2. Check for App Handlers (Legacy / Local tools)
        if (this.appHandlers[step.tool]) {
          console.log(`[ToolAgent] Using local app handler for: ${step.tool}`);
          try {
            const output = await this.appHandlers[step.tool](step.action, step.params);
            result = { success: true, data: output };
          } catch (e) {
            result = { success: false, error: e.message };
          }
        } 
        // 3. Map tool name to Arcade tool if applicable
        else {
          const arcadeToolName = this.mapToArcadeTool(step.tool);
          if (arcadeToolName && typeof arcadeToolName === 'object' && arcadeToolName.type === 'local') {
            const handler = this.appHandlers[arcadeToolName.handler];
            if (typeof handler === 'function') {
              const output = await handler(step.action, step.params);
              result = { success: true, data: output };
            } else {
              result = { success: false, error: `local_handler_missing:${arcadeToolName.handler}` };
            }
          } else if (arcadeToolName) {
