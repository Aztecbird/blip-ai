...
export function getTelegramVoiceCommand(command = '') {
    ...
    const politePrefix = '(?:(?:blip\\s+)?(?:i\\s+want\\s+(?:you\\s+)?to|can\\s+you|could\\s+you|will\\s+you|please)\\s+)?';
    ...
    const naturalPhotoTargetMatch = lower.match(
        new RegExp(`^${politePrefix}(?:send|share)\\s+(?:this|the|current|open)?\\s*(?:photo|picture|image|snapshot|shot|it)\\s+(?:to|for)\\s+(.+)$`)
    );
    ...
    if (
        new RegExp(`^${politePrefix}(?:send|share)\\s+(?:(?:this|the|latest|last|current|open|my)\\s+)?(?:photo|picture|image|snapshot|shot)(?:\\s+on|\\s+in|\\s+via|\\s+with|\\s+over)?\\s+telegram$`).test(lower)
    ...
    const youtubeShareNoun = '(?:(?:youtube\\s+)?(?:video|link)|youtube)';
    const explicitTelegramYouTubeTargetMatch = lower.match(
        new RegExp(`^${politePrefix}(?:send|share)\\s+(?:this|that|the|current|open)?\\s*${youtubeShareNoun}(?:\\s+link)?\\s+(?:on|in|via|with|over|to)\\s+telegram(?:\\s+(?:to|for)\\s+(.+))?$`)
    );
    if (explicitTelegramYouTubeTargetMatch) {
        return {
            action: 'shareCurrent',
            shareType: 'youtube',
            quickSend: lower.includes('send ') || lower.startsWith('send'),
            draft: { chatId: String(explicitTelegramYouTubeTargetMatch[1] || '').trim() }
        };
    }

    const reversedTelegramYouTubeTargetMatch = lower.match(
        new RegExp(`^${politePrefix}(?:send|share)\\s+(?:this|that|the|current|open)?\\s*${youtubeShareNoun}(?:\\s+link)?\\s+(?:to|for)\\s+(.+?)\\s+(?:on|in|via|with|over)\\s+telegram$`)
    );
    if (reversedTelegramYouTubeTargetMatch?.[1]) {
        return {
            action: 'shareCurrent',
            shareType: 'youtube',
            quickSend: lower.includes('send ') || lower.startsWith('send'),
            draft: { chatId: String(reversedTelegramYouTubeTargetMatch[1] || '').trim() }
        };
    }

    if (/^telegram\s+(?:this|that|the|current|open)?\s*(?:youtube\s+)?(?:video|link|youtube)(?:\s+link)?$/.test(lower)) {
        return {
            action: 'shareCurrent',
            shareType: 'youtube',
            quickSend: false,
    ...
    const explicitTelegramPhotoTargetMatch = lower.match(
        new RegExp(`^${politePrefix}(?:send|share)\\s+(?:this|the|latest|last|current|open|my)?\\s*(?:photo|picture|image|snapshot|shot|it)\\s+(?:on|in|via|with|over|to)\\s+telegram(?:\\s+group)?\\s+(?:to|for)?\\s*(.+)$`)
    );
    ...
    const conversationalTelegramPhotoTargetMatch = lower.match(
        /(?:^|.*\b)(?:send|share)\s+(?:this|the|latest|last|current|open)?\\s*(?:photo|picture|image|snapshot|shot|it)\\s+(?:on|in|via|with|over)\\s+telegram\\s+(?:to|for)\\s+(.+)$/
    );
    ...
    if (/^(?:connect)\s+telegram$/.test(lower)) {
        return { action: 'connect' };
    }
    if (/^(?:disconnect)\s+telegram$/.test(lower)) {
        return { action: 'disconnect' };
    }

    if (
        /^(?:telegram\s+instead|use\s+telegram|not\s+(?:email|gmail)\s+telegram)$/.test(lower)
        || /^(?:send\s+(?:it|that|this|the\s+message)\s+(?:in|on|via)\s+telegram)$/.test(lower)
    ) {
        return {
            action: 'compose',
            draft: { chatId: '', text: '' },
    ...
    const reversedTargetedMatch = lower.match(
        new RegExp(`^${politePrefix}(send|write|compose)(?:\\s+(?:a|the))?\\s+(?:message\\s+)?(?:to|for)\\s+(.+?)\\s+(?:in|on|via|with|over|to)\\s+telegram(?:\\s+(?:a|the)\\s+message?)?(?:\\s+(?:saying|that\\s+says|message|with)\\s+(.+))?$`)
    );
