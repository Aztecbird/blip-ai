...
BLIP_ENABLE_HUME_BACKEND="${BLIP_ENABLE_HUME_BACKEND:-}"
...
BLIP_ENABLE_EXPENSE_TRACKER_BACKEND="${BLIP_ENABLE_EXPENSE_TRACKER_BACKEND:-1}"
...
if [ "$BLIP_ENABLE_HUME_BACKEND" = "1" ]; then
  start_bg "Hume backend" "${HUME_BACKEND_PORT:-8796}" "$LOG_DIR/hume-backend.log" npm run dev:hume-backend
fi

if [ "$BLIP_ENABLE_EXPENSE_TRACKER_BACKEND" = "1" ]; then
  start_bg "Expense tracker backend" "${EXPENSE_TRACKER_BACKEND_PORT:-8797}" "$LOG_DIR/expense-tracker-backend.log" npm run dev:expense-tracker-backend
fi
